...
def get_PDF(
    ...
    if np.all(np.diff(val) > 0):
        ...
        pdf_grid /= np.trapezoid(pdf_grid, grid)
    ...
    # Censor the PDF if a lower/upper-limit is provided
...
def censor_distribution(
    ...
    if (
        ...
        truncated_mass = 1 - np.trapezoid(censored_dist, x)
        if truncated_mass > 0:
    ...
    if (
        ...
        if truncated_mass > 0:
            if boundary_dist.lower() == 'delta':
                ...
                raise NotImplementedError(f'{boundary_dist} is not implemented')
    censored_dist /= np.trapezoid(censored_dist, x)
...
def histogram(data, bins, density=False):
    ...
    if density:
        counts = counts[:, :-1] / np.trapezoid(counts[:, :-1], bin_centers, axis=1)[:, np.newaxis]
    return counts, bins
